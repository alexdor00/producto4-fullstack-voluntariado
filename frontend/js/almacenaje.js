...
//codesandbox
const API_URL = "https://kt5td5-4000.csb.app/graphql";
...
function inicializarWebSocket() {
  ...
  //codesandbox
  socket = io("https://kt5td5-4000.csb.app", {
    reconnection: true,
  ...
  socket.on("voluntariado_creado", (voluntariado) => {
    ...
    window.dispatchEvent(
      new CustomEvent("voluntariado-creado", {
        detail: voluntariado,
      })
    );
  });
  ...
  socket.on("voluntariado_actualizado", (voluntariado) => {
    ...
    limpiarCache("voluntariados");
    window.dispatchEvent(
      new CustomEvent("voluntariado-actualizado", {
        detail: voluntariado,
      })
    );
  });
  ...
  socket.on("voluntariado_eliminado", (data) => {
    ...
    limpiarCache("voluntariados");
    window.dispatchEvent(
      new CustomEvent("voluntariado-eliminado", {
        detail: data,
      })
    );
  });
...
function intentarReconexion() {
  ...
  intervaloReconexion = setInterval(() => {
    intentosReconexion++;
    console.log(
      `[websocket] intento de reconexion ${intentosReconexion}/${MAX_INTENTOS_RECONEXION}`
    );
...
export async function guardarSeleccion(voluntariadosSeleccionados) {
  try {
    localStorage.setItem(
      "seleccion",
      JSON.stringify(voluntariadosSeleccionados)
    );
    console.log(
      "[dashboard] seleccion guardada:",
      voluntariadosSeleccionados.length
    );
    return { ok: true };
  } catch (error) {
